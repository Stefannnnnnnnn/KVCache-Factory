...
class SnapKVCluster():
    def __init__(self, window_size = 64, max_capacity_prompt = 256 + 64, kernel_size = 5, pooling = 'avgpool', merge = None, recent_size = 32, ratio =  0.4, chunk_size = 1024, low_precision_dtype = torch.float16):
        self.window_size = window_size
        self.max_capacity_prompt = max_capacity_prompt
        assert self.max_capacity_prompt - self.window_size > 0
        self.kernel_size = kernel_size
    ...
    def reset(self, window_size = 64, max_capacity_prompt = 256 + 64, kernel_size = 5, pooling = 'avgpool', merge = None, chunk_size = 1024, low_precision_dtype = torch.float16):
        self.window_size = window_size
        self.max_capacity_prompt = max_capacity_prompt
        assert self.max_capacity_prompt - self.window_size > 0
        self.kernel_size = kernel_size
...
def init_snapkv(self):
    ...
    self.config.snapkv_chunk_size = 1024 # Default chunk size
    self.config.snapkv_low_precision_dtype = torch.float16 # Default low precision
    ...
    self.kv_cluster = SnapKVCluster( 
        ...
        pooling = self.config.pooling,
        merge = self.config.merge, 
        chunk_size = self.config.snapkv_chunk_size, 
        low_precision_dtype = self.config.snapkv_low_precision_dtype
    )
