...
# --- Configuration & Data ---
# IMPORTANT: Replace with your actual long context passages for hotpotqa samples
LONG_CONTEXT_1 = """Passage 1:
...
# Delimiters for specific parsing logic
STREAM_START_DELIMITER = "</think><think>"
FINAL_ANSWER_END_DELIMITER = "</think>"
...
# Expected max new tokens for generation
DATASET2MAXLEN = {
    "hotpotqa": 3950, # User-defined max length for HotpotQA
}

DEFAULT_MODEL_PATH = "Annieee215/Deepseek-R-Distilled-Llama-70B-awq-shortGPT5"

PREDEFINED_DATASET_NAME = "hotpotqa"
FIXED_TEST_CASES = {
    "hotpotqa": [
        {
            "_id": "hotpotqa_predefined_sample_1", # Made ID more specific
            "context": LONG_CONTEXT_1,
...
def load_model_and_tokenizer(model_path):
    ...
    try:
        ...
        try:
            ...
            st.info("Successfully imported 'replace_llama' from pyramidkv.monkeypatch.")

            # This is a basic check; more robust checks might be needed for other model types
            if "llama" in model_path.lower() or "deepseek" in model_path.lower():
                replace_llama("FullKV".lower()) 
                st.info(f"PyramidKV 'replace_llama' patch applied for model type: {model_path}.")
                # If HQQQuantizedCache setup is essential for your PyramidKV version with "quantized" HQQ backend:
                # try:
                #     from pyramidkv.quantcache import KVQuantizedCache
                #     from transformers import cache_utils
                #     cache_utils.HQQQuantizedCache = KVQuantizedCache
                #     st.info("PyramidKV HQQQuantizedCache setup successful.")
                # except Exception as hqq_e:
                #     st.warning(f"PyramidKV: Could not set HQQQuantizedCache: {hqq_e}")
                use_pyramidkv_flag = True 
            else:
                st.info("PyramidKV 'replace_llama' patch not attempted (model name does not suggest Llama/DeepSeek).")

        except ImportError:
            st.warning("PyramidKV not found or 'replace_llama' could not be imported. Running without PyramidKV-specific cache optimizations.")
        except Exception as e:
            st.warning(f"Error during PyramidKV setup ('replace_llama' or other): {e}. Running without PyramidKV-specific cache optimizations.")
        ...
        model.eval()
        st.success(f"Model '{model_path}' loaded successfully on {str(actual_device).upper()}!")
        return model, tokenizer, actual_device, use_pyramidkv_flag
    except Exception as e:
...
st.title("""📝 Demo: Run Compressed Llama-3 on a Single A40""")

st.sidebar.header("⚙️ Model Configuration")
selected_model_path_input = st.sidebar.text_input("Hugging Face Model Path", value=DEFAULT_MODEL_PATH, help="e.g., meta-llama/Llama-2-7b-chat-hf")
st.sidebar.text_input("KV Cache Sparicification", value="PyramidKV-1024", help="e.g., FullKV")
st.sidebar.text_input("KV Cache Quantization", value="kivi-8bit", help="e.g., 8-bit kvquant")

if 'model_loaded' not in st.session_state:
    st.session_state.model_loaded = False
if 'model' not in st.session_state: # Initialize all related model state variables together
    st.session_state.model = None
    st.session_state.tokenizer = None
    st.session_state.device = None
    st.session_state.use_pyramidkv = False # Initialize in session state

if selected_model_path_input and not st.session_state.model_loaded:
    model_obj, tokenizer_obj, device_obj, use_py_kv_flag = load_model_and_tokenizer(selected_model_path_input)
    if model_obj and tokenizer_obj and device_obj:
        st.session_state.model = model_obj
        st.session_state.tokenizer = tokenizer_obj
        st.session_state.device = device_obj
        st.session_state.use_pyramidkv = use_py_kv_flag 
        st.session_state.model_loaded = True
    else: # Explicitly set all to None on failure to avoid partial states
        st.session_state.model_loaded = False
        st.session_state.model = None
        st.session_state.tokenizer = None
        st.session_state.device = None
        st.session_state.use_pyramidkv = False


model = st.session_state.get('model', None)
tokenizer = st.session_state.get('tokenizer', None)
device = st.session_state.get('device', None)
# This flag will determine if KV cache quantization is attempted during generation
use_pyramidkv_on_generate = st.session_state.get('use_pyramidkv', False) 

st.session_state.setdefault('current_sample_display_info', None)
st.session_state.setdefault('results', None)
st.session_state.setdefault('full_prompt_for_display', None)


st.header("🚀 Benchmark Test: Longbench(HotpotQA)")
benchmark_col1, benchmark_col2 = st.columns([2,1]) # Adjusted column ratio for more space on left

with benchmark_col1:
    st.subheader("🧪 Test Configuration & Control")
    st.markdown(f"**Dataset:** `{PREDEFINED_DATASET_NAME}` (Hardcoded)")
    
    # Ensure PREDEFINED_DATASET_NAME is valid before using it as a key
    if PREDEFINED_DATASET_NAME in MODEL2PROMPT:
        prompt_template_display = MODEL2PROMPT[PREDEFINED_DATASET_NAME].replace("{context}", "{CONTEXT_PLACEHOLDER}").replace("{input}", "{INPUT_PLACEHOLDER}")
        st.text_area("Prompt Template:", prompt_template_display, height=100, disabled=True)
    else:
        st.error(f"Prompt template for '{PREDEFINED_DATASET_NAME}' not found in MODEL2PROMPT.")

    if st.button("🚀 Run Sample Test", type="primary", disabled=not st.session_state.model_loaded):
        st.session_state.results = None
        st.session_state.current_sample_display_info = None
        st.session_state.full_prompt_for_display = None # Clear previous full prompt
        sample, sample_loaded_successfully = None, False

        try:
            if PREDEFINED_DATASET_NAME in FIXED_TEST_CASES and PREDEFINED_SAMPLE_INDEX < len(FIXED_TEST_CASES[PREDEFINED_DATASET_NAME]):
                sample = FIXED_TEST_CASES[PREDEFINED_DATASET_NAME][PREDEFINED_SAMPLE_INDEX]
                sample_loaded_successfully = True
                st.info(f"Loaded predefined sample '{sample.get('_id', 'N/A')}' for '{PREDEFINED_DATASET_NAME}'.")
            else:
                st.error(f"Error: Predefined sample ({PREDEFINED_DATASET_NAME}, index {PREDEFINED_SAMPLE_INDEX}) configuration is invalid.")
        except Exception as e:
            st.error(f"Error loading predefined sample: {e}")
        
        if sample_loaded_successfully and sample and model and tokenizer and device:
            if PREDEFINED_DATASET_NAME not in MODEL2PROMPT:
                st.error(f"Cannot run test: Prompt template for '{PREDEFINED_DATASET_NAME}' is missing.")
            else:
                prompt_template = MODEL2PROMPT[PREDEFINED_DATASET_NAME]
                full_prompt = prompt_template.format(context=sample.get("context", ""), input=sample.get("input", "N/A"))
                st.session_state.full_prompt_for_display = full_prompt # Store for expander

                input_display_parts = [
                    f"**Test Set:** {PREDEFINED_DATASET_NAME}",
                    f"**Sample ID:** {sample.get('_id', 'N/A')}",
                    (f"**Context (truncated):**\n```\n{truncate_text(sample.get('context', ''), 150)}\n```\n" if sample.get("context") else None), # Conditional context
                    f"**Question/Input:** {sample.get('input', '')}"
                ]
                st.session_state.current_sample_display_info = "\n\n".join(filter(None, input_display_parts))
                st.session_state.results = {}

                st.subheader("⚡️ Filtered Streaming Output") # Updated subheader
                streaming_container = st.container(border=True) # Added border for better visibility
                stream_placeholder = streaming_container.empty()
                full_generated_text = ""
                
                spinner_message = f"Processing with '{selected_model_path_input}' on {device}..."
                if use_pyramidkv_on_generate:
                    spinner_message += " (PyramidKV cache optimizations potentially active)"
                else:
                    spinner_message += " (Standard cache)"

                with st.spinner(spinner_message + ". Streaming output below."):
                    try:
                        tokenized_prompts = tokenizer(full_prompt, return_tensors="pt", add_special_tokens=True).to(device)
                        actual_input_token_count = tokenized_prompts.input_ids.shape[1]
                        max_new_tokens = DATASET2MAXLEN.get(PREDEFINED_DATASET_NAME, 256) # Default 256 if not in map
                        ...
                        if use_pyramidkv_on_generate:
                            st.info("Attempting to use PyramidKV quantized cache.")
                            gen_kwargs["cache_implementation"] = "quantized"
                            gen_kwargs["cache_config"] = {
                                "nbits": 8, "backend": "HQQ", "device": str(device),
                                "residual_length": max_new_tokens, "axis_key": 1, "q_group_size": 64
                            }
                        ...
                        displayed_text_in_stream = ""
                        initial_wait_message = "<i>(Waiting for model's thinking process (post-<code>&lt;/think&gt;&lt;think&gt;</code>) to start streaming...)</i>"
                        stream_placeholder.caption(initial_wait_message) # Initial message using st.caption

                        for new_text_chunk in streamer:
                            full_generated_text += new_text_chunk
                            
                            start_delimiter_pos = full_generated_text.find(STREAM_START_DELIMITER)
                            if start_delimiter_pos != -1:
                                displayed_text_in_stream = full_generated_text[start_delimiter_pos + len(STREAM_START_DELIMITER):]
                                stream_placeholder.markdown(displayed_text_in_stream + "▌", unsafe_allow_html=True)
                            else:
                                # Keep showing waiting message if delimiter not yet found
                                # The placeholder is already showing the initial wait message
                                pass 
                        
                        thread.join()
                        time_end_generate = time.perf_counter()
                        
                        final_display_text_for_stream = ""
                        final_start_delimiter_pos = full_generated_text.find(STREAM_START_DELIMITER)
                        if final_start_delimiter_pos != -1:
                            final_display_text_for_stream = full_generated_text[final_start_delimiter_pos + len(STREAM_START_DELIMITER):]
                        else:
                             final_display_text_for_stream = f"<i>(Streaming delimiter <code>{STREAM_START_DELIMITER}</code> not found in the output.)</i>"
                        stream_placeholder.markdown(final_display_text_for_stream, unsafe_allow_html=True) # Final update without cursor

                        num_generated_tokens = len(tokenizer.encode(full_generated_text, add_special_tokens=False))
                        
                        final_model_answer = full_generated_text 
                        end_delimiter_pos = full_generated_text.find(FINAL_ANSWER_END_DELIMITER)
                        if end_delimiter_pos != -1:
                            final_model_answer = full_generated_text[:end_delimiter_pos].strip()
                        else:
                            st.warning(f"Final answer delimiter '<code>{FINAL_ANSWER_END_DELIMITER}</code>' not found. The 'Extracted Final Answer' will be the full generated text.", icon="⚠️")
                            
                        latency_seconds = time_end_generate - time_start_generate
                        tokens_per_second = ((actual_input_token_count + num_generated_tokens) / latency_seconds) if latency_seconds > 0 and num_generated_tokens > 0 else 0.0
                        
                        st.session_state.results.update({
                            "model_answer_raw": full_generated_text,
                            "model_answer_final": final_model_answer,
                            "ground_truth": "\n".join(sample["answers"]) if isinstance(sample.get("answers"), list) else sample.get("answers", "N/A"),
                            "context_length_tokens": actual_input_token_count,
                            "generated_tokens": num_generated_tokens,
                            "latency_sec": f"{latency_seconds:.3f}",
                            "tokens_per_sec_gen_only": f"{tokens_per_second:.2f}",
                        })
                        st.success("Streaming and processing complete!")

                    except Exception as e:
                        st.error(f"Error during model generation or streaming: {e}\n{traceback.format_exc()}")
                        if st.session_state.results is None: st.session_state.results = {}
                        st.session_state.results.update({"error": str(e)})
            
        elif not (model and tokenizer and device):
            st.warning("Please load a model to enable testing.")
        elif not sample_loaded_successfully: # This case might be redundant due to earlier checks
            st.warning("Failed to load the predefined sample. Check configuration.")

    # Display sample info if available
    if st.session_state.current_sample_display_info:
        st.markdown("---"); st.subheader("📋 Sample Details")
        st.markdown(st.session_state.current_sample_display_info, unsafe_allow_html=True)
        if st.session_state.get('full_prompt_for_display'):
            with st.expander("View Full Prompt Sent to Model"):
                st.text_area("Full Prompt:", st.session_state.full_prompt_for_display, height=200, disabled=True, key="full_prompt_display_area")
    elif not st.session_state.current_sample_display_info and st.session_state.model_loaded:
        st.info("Click the 'Run Single Sample Test' button to start.")
    elif not st.session_state.model_loaded:
        st.warning("Please enter a valid model path in the sidebar and wait for the model to load.")

with benchmark_col2:
    st.subheader("📊 Test Results")
    if st.session_state.results:
        if "error" in st.session_state.results and "model_answer_final" not in st.session_state.results:
            st.error(f"Processing failed: {st.session_state.results['error']}")
        elif "model_answer_final" in st.session_state.results:
            st.markdown(f"**🤖 Model's Extracted Final Answer:**")
            st.info(st.session_state.results["model_answer_final"]) # Use st.info for better visibility
            ...
            st.markdown(f"**💡 Generated Tokens (full output):** `{st.session_state.results['generated_tokens']}`")
            st.markdown(f"**⏱️ Latency:** `{st.session_state.results['latency_sec']}` seconds")
            st.markdown(f"**⚡ Tokens/Sec (Generation):** `{st.session_state.results['tokens_per_sec_gen_only']}`")
            with st.expander("See Full Raw Model Output (includes all parts)"): # Clarified title
                st.text_area("Raw Output:", st.session_state.results.get("model_answer_raw", "N/A"), height=200, disabled=True, key="raw_output_display_area")
        else: 
            st.caption("Results will appear here after processing.") # Changed to caption
    else: 
        st.caption("Test results will be displayed here.") # Changed to caption